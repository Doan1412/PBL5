...
export default function SidebarProfile({ data }: PropsSidebar) {
  return (
    <>
      <div className="bg-[#ffffff] dark:bg-[#242526] rounded-lg mb-4 shrink drop-shadow-2xl dark:text-white">
        ...
        <div className="mt-5 ml-5 flex">
          ...
          <h1 className="ml-1">
            Từng học tại{" "}
            <span className="font-medium dark:text-white">{data?.study_at}</span>
          </h1>
        </div>
        ...
        <div className="mt-3 ml-5 mr-2 flex">
          <IoHeart size={25} />
          <h1 className="ml-1">
            <span className="ml-1 font-medium dark:text-white">{data?.relationship}</span>
          </h1>
        </div>
