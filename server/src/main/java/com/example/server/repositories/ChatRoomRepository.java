...
public interface ChatRoomRepository extends Neo4jRepository<ChatRoom, String> {
    ...
    @Query("MATCH (r:room{id: $id})-[:RECIPIENT_MESSAGE]->(m:message)<-[:SENT_MESSAGE]-(u:User) RETURN m.id AS id, m.content AS content, m.timestamp AS timestamp, u.id AS senderId, r.id AS roomId ORDER BY m.timestamp ASC SKIP $page LIMIT $size")
